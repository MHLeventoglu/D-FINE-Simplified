    ...
    **kwargs,
    ...
    if use_wandb:
        ...
        for k, v in metrics.items():
            if k == "extended_metrics" and isinstance(v, dict):
                # Flatten extended metrics (per-class precision, recall, iou)
                for ext_k, ext_v in v.items():
                    if not (isinstance(ext_v, float) and (ext_v != ext_v)):  # skip NaN
                        wandb_metrics[f"metrics/per_class/{ext_k}"] = ext_v
            else:
                wandb_metrics[f"metrics/{k}"] = v
